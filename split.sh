...
function cleanup() {
    ( cd new-repos/TEMP-$1 &&
        ...
        git filter-branch --tag-name-filter cat --commit-filter '/mnt/ramdisk/remove-pointless-commit.rb "$@"' master ${branches} &&
        git for-each-ref --format="%(refname)" refs/original/ | xargs -n 1 git update-ref -d &&
...
( cd new-repos/TEMP-brooklyn-docs && git filter-branch --index-filter "git rm -q -r --cached --ignore-unmatch $( cat ../../*-whitelist.txt | tr '\n' ' ' ); done" --tag-name-filter cat --prune-empty master ${branches} )
